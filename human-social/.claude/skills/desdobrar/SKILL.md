---
name: desdobrar
description: Unfolds a folder with text + images into native pieces for Instagram Feed, Instagram Stories, and LinkedIn Feed via Higgsfield CLI + GPT Image 2 with reference chain. Conversational pre-flight for CLI/login (user never touches a file). Use when the user types `/desdobrar <folder>` or asks to "unfold carousel" / "create versions for each platform" from local material.
---

# /desdobrar — Pipeline with zero-tech onboarding
...
Resolve `HUMAN_AGENT_LAB_HOME` before running any command. The script does not live in the user's current project; it lives inside the central install:
...
Run:
```bash
python3 "$HUMAN_AGENT_LAB_HOME/Human Social/scripts/desdobrar.py" prep "<folder>"
```
...
Then run:
```bash
python3 "$HUMAN_AGENT_LAB_HOME/Human Social/scripts/desdobrar.py" generate "<folder>" ig-feed "<folder>/desdobramento/_prompts/ig-feed.txt" --base <master-art.png>
```
...
for N in 01 02 03; do
  python3 "$HUMAN_AGENT_LAB_HOME/Human Social/scripts/desdobrar.py" generate "<folder>" ig-stories "<folder>/desdobramento/_prompts/story-$N.txt" --base "<master-art.png>" --output "story-$N.png" &
done
wait
```
...
Run:
```bash
python3 "$HUMAN_AGENT_LAB_HOME/Human Social/scripts/desdobrar.py" generate "<folder>" linkedin-feed "<folder>/desdobramento/_prompts/linkedin-feed.txt" --base <master-art.png>
```
...
```bash
python3 "$HUMAN_AGENT_LAB_HOME/Human Social/scripts/desdobrar.py" presentation-pdf "<folder>"
```
...
- **GPT Image 2 ALWAYS.** Visual unfoldings are `higgsfield generate create gpt_image_2`.
