...
Takes a folder with text + images, generates native pieces for Instagram Feed, Instagram Stories, and LinkedIn Feed via Higgsfield CLI + **GPT Image 2** (`gpt_image_2`). Each piece is a **direct unfolding of the master art**: same photo/visual, same font language, same colors, same graphic elements, with format adaptation, copy, and safe zones.
...
│   └── skills/desdobrar/SKILL.md      full pipeline + inline prompts
├── .Codex/skills/desdobrar/SKILL.md   compatibility wrapper
...
python3 "$HUMAN_AGENT_LAB_HOME/Human Social/scripts/desdobrar.py" prep <folder>                            # scaffold + lists inputs
python3 "$HUMAN_AGENT_LAB_HOME/Human Social/scripts/desdobrar.py" generate <folder> <format> <prompt> ...  # calls Higgsfield CLI + downloads
python3 "$HUMAN_AGENT_LAB_HOME/Human Social/scripts/desdobrar.py" presentation-pdf <folder>                # final delivery PDF
...
If not set, the script uses `gpt_image_2`, `high` quality, and `2k` resolution. This is the mandatory model for all Human Social unfoldings, because the final pieces have lettering, design, and text rendered together with the image.
...
- **Always GPT Image 2.** Every Human Social visual unfolding uses `gpt_image_2`.
