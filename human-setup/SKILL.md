...
- Higgsfield: run a non-destructive identity check in the shell, e.g. higgsfield whoami (or higgsfield --version to test install, then whoami to test login). If it prints a user, Higgsfield is ready, skip its setup.
...
2. Install the Higgsfield CLI globally: npm install -g higgsfield (use the package name from the Higgsfield docs if it differs).
3. Log in: higgsfield login and follow the browser/device prompt.
4. Verify: higgsfield whoami.

### Windows

1. Ensure Node.js LTS is installed from nodejs.org, then check node --version in PowerShell.
2. Install the CLI: npm install -g higgsfield.
3. Log in: higgsfield login.
4. Verify: higgsfield whoami.
