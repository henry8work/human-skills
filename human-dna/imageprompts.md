...
> Single, universal guide to generate high-end cinematic images. The user **does not choose** camera, lens, light or mood — the system decides like a director of photography. The mandatory render destination is **Higgsfield CLI** using **Nano Banana 2 (`nano_banana_2`)**.
...
`/image` does not end at the prompt. The default flow is always Higgsfield CLI + Nano Banana 2 (`nano_banana_2`):

1. Understand the request.
2. Ask only what's missing for the render: **aspect ratio** and **size/resolution**.
3. Generate the final prompt.
4. Save the prompt to `Human Images/output/{run_id}/prompt.txt`.
5. Render via Higgsfield CLI with `Human Images/scripts/render_image.py`.
...
```bash
python3 "Human Images/scripts/render_image.py" render "Human Images/output/{run_id}/prompt.txt" --aspect-ratio "4:5" --resolution "2k" --output-dir "Human Images/output/{run_id}" --reference "/path/to/reference.png"
```
