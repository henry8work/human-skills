...
All relative paths below resolve against the **human-team skill directory**: `${CLAUDE_PLUGIN_ROOT}/skills/human-team/` when running as an installed plugin (or the `human-team/` folder itself in a local checkout). Treat that directory as **read-only intelligence**.

**Mutable state never goes inside the plugin directory.** Memories, run history, dashboard state and company config live in the user's project at `<cwd>/human-output/team/_state/`:

- `<cwd>/human-output/team/_state/memories.md` — seed from `squads/team/_memory/memories.md` on first run
- `<cwd>/human-output/team/_state/runs.md` — seed from `squads/team/_memory/runs.md` on first run
- `<cwd>/human-output/team/_state/state.json` — seed from `squads/team/state.json` on first run
- `<cwd>/human-output/team/_state/preferences.md` and `company.md` — created when collected

Read seeds from the skill dir; write only to `_state/`.
...
- `<cwd>/human-output/team/_state/memories.md` (seed: `squads/team/_memory/memories.md`)
- `<cwd>/human-output/team/_state/runs.md` (seed: `squads/team/_memory/runs.md`)
- `<cwd>/human-output/team/_state/preferences.md`, if it exists
- `<cwd>/human-output/team/_state/company.md`, if it exists
- `_opensquad/core/runner.pipeline.md`

If `company.md` does not exist in `_state/`, treat it as a pending configuration. Collect the minimum needed to proceed and save the file before running the pipeline.
...
Before starting any time-consuming analysis, update the dashboard in `<cwd>/human-output/team/_state/state.json`: squad status `running`, label `Receiving demand`, `startedAt` filled, and at least one relevant agent marked as `working` (usually Planner; if there is immediate research, Scout too; if there is visual material, Art Director too). Never leave all agents as `idle` while you are working.
...
6. Check that `<cwd>/human-output/team/_state/company.md` exists. If not, create it from the user's answers.
...
- Company, brand, product, audience, official channels, tone of voice and permanent prohibitions: `<cwd>/human-output/team/_state/company.md`
- User operation preferences: `<cwd>/human-output/team/_state/preferences.md`
- Specific learnings of the Creative Team: `<cwd>/human-output/team/_state/memories.md`
- Run history: `<cwd>/human-output/team/_state/runs.md`
...
```bash
# TEAM_DIR = the human-team skill directory (${CLAUDE_PLUGIN_ROOT}/skills/human-team)
# Requires playwright in the user's project: npm install playwright (once, in <cwd>)
NODE_PATH="$(pwd)/node_modules" node "$TEAM_DIR/scripts/render-project-document.js" "Campanhas/{campaign_slug}"
```

If `playwright` is not installed in the user's project, install it there first (`npm install playwright && npx playwright install chromium`) — never install dependencies inside the plugin directory.
...
- `<cwd>/human-output/team/_state/runs.md`
- `<cwd>/human-output/team/_state/memories.md`, if there is reusable learning
- `<cwd>/human-output/team/_state/state.json`, as per the runner
