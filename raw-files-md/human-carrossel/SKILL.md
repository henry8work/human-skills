...
- **Visual render: always GPT Image 2 — never Nano Banana 2 here.** Ask the user which render path: 1) **Magnific direto** — `images_generate` with `mode: "gpt-2"` (plan credits); 2) **Magnific Híbrido (Run Unlimited)** — Space nodes with `mode: "gpt-2"`, the user clicks Run Unlimited in the browser (zero credits, only if the model is unlimited-eligible on their plan); 3) **Higgsfield CLI** — `gpt_image_2`, the original tuned default; 4) **ChatGPT manual** — you write one ready-to-paste image prompt per slide (built from the editorial spine, headline, copy and references), the user generates each in their own ChatGPT (GPT Image, zero cost), saves them into `slides/` as `01.png`, `02.png`, … and you resume the pipeline (verification, assembly, delivery). For paths 1–3 always pass references as `--image`/`references[]` when they exist; for path 4, describe the references inside the prompt and tell the user to attach them in ChatGPT.
