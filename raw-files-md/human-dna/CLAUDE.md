...
> **The system is multi-tenant.** Each project/brand lives in an isolated subfolder inside `projetos/`. **Path resolution:** `projetos/` is `<cwd>/human-output/dna/` in the user's project (never inside the plugin directory); `inteligencias/`, `imageprompts.md` and `scripts/` resolve against this skill's base directory (`$SKILL_DIR`). All interaction happens **inside a project** — never in the root folder.
>
...
1. Run the local collector:
   ```bash
   python3 "$SKILL_DIR/scripts/collect-instagram.py" --profile "@handle" --project "projetos/[slug]" --limit 50
   ```
2. If the profile requires login or blocks public reading, try again with interactive login:
   ```bash
   python3 "$SKILL_DIR/scripts/collect-instagram.py" --profile "@handle" --project "projetos/[slug]" --limit 50 --login "instagram_user"
   ```
...
**Kernel** (`02. DNA Criativo/`, permanent, doesn't change between projects): `COMECE-AQUI.md` (human guide), `CLAUDE.md` (you), `inteligencias/` (19 files: doctors + operational + `_template/` project model), `projetos/` (brand container).
