...
> **⚠️ CASE STUDY — methodology reference only.** This is a scene-by-scene breakdown of a public Nubank ad, used to teach the decupagem method. The asset files it mentions (a `nubank/` folder, logos, frames) are **NOT included** in this distribution. Do not try to reproduce or render this document — extract the method (structure, timing tables, layer logic) and apply it to the user's own material.
