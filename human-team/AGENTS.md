...
- When a run generates files, always report the final output folder as a clickable absolute-path link and list every generated non-`.md` file as clickable absolute-path links. Do not list `.md` files individually unless the user asks, but always include the final folder so the user can find them.
